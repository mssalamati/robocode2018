...
public class DangerBot {
	...
	Hashtable<String, KDTree.WeightedManhattan<Scanner>> targets;
	KDTree.WeightedManhattan<Scanner> defaultAim;

	FireManager bulletPowerPredictor;
	...
	double[] targetDescriptor() {
		...
		return new double[] { // value normalize weighting
				Math.abs(latVel) * (1 / 8.0) * 10, (accel + 2) * (1 / 3.) * 3, (advVel + 8) * (1 / 16.0) * 2,
				distLast10 * (1 / 80.) * 2, 1. / (1 + 2 * timeSinceReverse / BFT) * (1) * 2, distToE * (1 / 1200.0) * 1,
				distToNearest * (1 / 1200.0) * 2, distToWall * (1 / 500.0) * 2, 1. / (1. + enemiesAlive) * (1.) * 4,
