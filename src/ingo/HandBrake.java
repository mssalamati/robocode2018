...
public class HandBrake {
	...
	public static final double sin(double d) {
		...
		// all values from "Fast Polynomial Approximations to Sine and Cosine", Garret,
		// C. K., 2012
		(((((-2.05342856289746600727e-08 * x2 + 2.70405218307799040084e-06) * x2 - 1.98125763417806681909e-04) * x2
				+ 8.33255814755188010464e-03) * x2 - 1.66665772196961623983e-01) * x2 + 9.99999707044156546685e-01) * d;
	}
	...
	public static final double cos(double d) {
		...
		// all values from "Fast Polynomial Approximations to Sine and Cosine", Garret,
		// C. K., 2012
		((((-2.21941782786353727022e-07 * d + 2.42532401381033027481e-05) * d - 1.38627507062573673756e-03) * d
				+ 4.16610337354021107429e-02) * d - 4.99995582499065048420e-01) * d + 1;
	}
	...
	public static final double cosInBounds(double d) {
		...
		// C. K., 2012
		((((-2.21941782786353727022e-07 * d + 2.42532401381033027481e-05) * d - 1.38627507062573673756e-03) * d
				+ 4.16610337354021107429e-02) * d - 4.99995582499065048420e-01) * d + 9.99999443739537210853e-01;
	}

	// Takes about 1/3 of the time of Math.tan
	// chebyshev polynomials calculated with
	...
	public static final double tan(double x) {
		...
		if (Math.abs(x) > 0.25 * Math.PI) {
			...
			i = x * x;
			return 1 / (x * (1 + i * (0.33555055109755784988404247864565
					+ i * (0.11611832804825573745353974789374 + i * (0.0939748057591306776926771332915)))));
		}

		i = x * x;
		return x * (1 + i * (0.33555055109755784988404247864565
				+ i * (0.11611832804825573745353974789374 + i * (0.0939748057591306776926771332915))));
	...
	private static final double chebyshev_atan(double x) {
		...
		return 0.46364760900080604 + xn * (0.4005785601195767
				+ xn * (-0.07982248463207417 + xn * (-0.007789235359791622 + xn * (0.00891247504621044))));
	}
