...
public class Radar {
	...
	public Radar(AdvancedRobot _bot) {
		bot = _bot;
	...
	public void onScannedRobot(ScannedRobotEvent e) {
		...
		eInfo.location = project(myLocation, otherAngle = bot.getHeadingRadians() + e.getBearingRadians(),
				e.getDistance());
		...
		if (bot.getOthers() <= enemies.size()) {
			...
			if (bot.getOthers() == 1 && oldestScan == eInfo.lastScanTime) {
				...
				bot.setTurnRadarRightRadians(Math.signum(angle)
						* limit(0, Math.abs(angle) + (Math.PI / 4 - Math.PI / 8 - Math.PI / 18), Math.PI / 4));

			} else
				bot.setTurnRadarRightRadians(Utils.normalRelativeAngle(otherAngle - bot.getRadarHeadingRadians())
