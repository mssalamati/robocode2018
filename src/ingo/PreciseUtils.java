...
public class PreciseUtils {
	...
	public static double[] getIntersectionRange(Point2D.Double botLocation, PreciseWave wave) {
		...
		for (int i = 0; i < 2; i++)
			for (int j = 0; j < 2; j++) {
				Point2D.Double[] testPoints = vertIntersect(wave.fireLocation.x, wave.fireLocation.y, radii[i],
						xBounds[j]);
		...
		for (int i = 0; i < 2; i++)
			for (int j = 0; j < 2; j++) {
				Point2D.Double[] testPoints = horizIntersect(wave.fireLocation.x, wave.fireLocation.y, radii[i],
						yBounds[j]);
				for (int k = 0; k < testPoints.length; k++)
		...
		return new double[] { fastAbsoluteAngle(antiClockAngle + absBearing),
				fastAbsoluteAngle(clockAngle + absBearing) };
	}
